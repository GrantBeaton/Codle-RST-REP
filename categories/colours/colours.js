...
let currentguess ="";
let word = 0;
let correctguess = colours[Math.floor(Math.random() * colours.length)]//Picks a random word from the category
...
if (colours && (guessesRemaining > 0)){//Calls the function if it has a word and if the user has enough guesses
    trackkey();
}
...
function checkGuess(guess) {
    if (!colours.includes(guess)) {//If a word is not in the category, false.
        console.log("Word not in the list.");
        return false;
    }
    if (!colours.includes(correctguess)) {// If the input is the right guess, returns
      return;
    }
