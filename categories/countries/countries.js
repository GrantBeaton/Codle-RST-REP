...
let currentguess ="";
let word = 0;
let correctguess = countries[Math.floor(Math.random() * countries.length)]//Picks a random word from the category
...
if (countries && (guessesRemaining > 0)){//Calls the function if it has a word and if the user has enough guesses
    trackkey();
}
...
function checkGuess(guess) {
    if (!countries.includes(guess)) {//If a word is not in the category, false.
        console.log("Word not in the list.");
        return false;
    }
    if (!countries.includes(correctguess)) {// If the input is the right guess, returns
      return;
    }
