...
let currentguess ="";
let word = 0;
let correctguess = food[Math.floor(Math.random() * food.length)]//Picks a random word from the category
...
if (food && (guessesRemaining > 0)){//Calls the function if it has a word and if the user has enough guesses
    trackkey();
}
...
function checkGuess(guess) {
    if (!food.includes(guess)) {//If a word is not in the category, false.
        console.log("Word not in the list.");
        return false;
    }
    if (!food.includes(correctguess)) {// If the input is the right guess, returns
      return;
    }
